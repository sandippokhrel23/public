...
<div class="wrapper col4">
  ...
  <?php
        ...
        <tr>
          ...
          <td>
    		<?php
    		$dcid=$_SESSION['doctorid'];
				$sqldoctor= "SELECT * FROM doctor INNER JOIN department ON department.departmentid=doctor.departmentid WHERE doctor.status='Active' AND doctor.doctorid='$dcid'";
        ...
        <tr>
          ...
          <td><input type="date" name="end_date" id="end_date" value="<?php echo $rsedit[prescriptionenddate]; ?>" /></td>
        </tr>

         <tr>
          <td width="34%">Treatment</td>
          <td width="66%"><select name="treatmentid" id="treatmentid">
          <option value="">Select</option>
            <?php
          	$sqltreatment= "SELECT * FROM treatment_records WHERE status='Active'";
			$qsqltreatment = mysqli_query($con,$sqltreatment);
			while($rstreatment = mysqli_fetch_array($qsqltreatment))
			{
				if($rstreatment[doctorid] == $_SESSION['doctorid'] && $rstreatment[patientid]==$_GET[patientid])
				{
				echo "<option value='$rstreatment[treatment_records_id]' selected>$rstreatment[treatment_description]</option>";
				}
			}
		  ?>
          </select></td>
